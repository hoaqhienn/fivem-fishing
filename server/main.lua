...
QBCore.Functions.CreateUseableItem("fishingrod", function(source, item)
    ...
    TriggerClientEvent('fishing:tryToFish', source)
end)

RegisterServerEvent('fishing:receiveFish')
AddEventHandler('fishing:receiveFish', function(cabin, house)
    local src = source
    ...
    if itemFound then
        for i = 1, itemCount, 1 do
			...
			elseif luck >= 55 and luck <= 60 then
                ...
                Player.Functions.AddItem(randomItem, 1)
				TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "add")
			elseif luck >= 25 and luck <= 55 then
				randomItem = "fish"
				itemInfo = QBCore.Shared.Items[randomItem]
                Player.Functions.AddItem(randomItem, 1)
				TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "add")
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
				if Player.PlayerData.items[k].name == "goldfish" then 
                ...
                elseif Player.PlayerData.items[k].name == "fish" then 
