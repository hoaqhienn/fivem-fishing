...
CastBait = function(rodHandle, castLocation)
    ...
    if caughtFish then
        TriggerServerEvent("fishing:receiveFish", castLocation, function(received)
        end)
...
TryToCatchFish = function()
    ...
    while true do
        ...
        DrawSprite(minigameSprites["tennisDict"], minigameSprites["tennisName"], 0.5, 0.4 + swingOffset, 0.018, 0.002,
            0.0, 0, 0, 0, 255)
...
IsInWater = function()
    ...
    local forwardPos =
        vector3(pedPos["x"] + forwardVector["x"] * 10, pedPos["y"] + forwardVector["y"] * 10, pedPos["z"])
...
SellFish = function()
    QBCore.Functions.TriggerCallback('fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("fishing:sellFish", function(sold, fishesSold)
        end)
    end)
end
